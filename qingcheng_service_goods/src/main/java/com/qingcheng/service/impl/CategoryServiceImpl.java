...
//        categoryMapper.insert(category);
    ...
    public void delete(Integer id) {
       //判断是否存在下级分类
        ...
        if (count>0){
            throw  new RuntimeException ("存在下一级分类不能进行删除");
        }
...
//        return findByParentId (categories,0);
        ...
        List<Map> o = (List <Map>) redisTemplate.boundValueOps (CacheKey.CATEGORY_TREE).get ();
        return  o;
    ...
    public void saveCategoryTreeToRedis() {
        //查询商品分类
        System.out.println ("加载商品分类数据到缓存中");
        Example example = new Example (Category.class);
        Example.Criteria criteria = example.createCriteria ();
        criteria.andEqualTo ("isShow","1");//显示
        List <Category> categories = categoryMapper.selectByExample (example);
    ...
    public List<Map> findByParentId(List<Category> categoryList,Integer parentId){
