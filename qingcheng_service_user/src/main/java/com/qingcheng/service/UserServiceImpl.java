...
import com.qingcheng.user.UserService;
...
public class UserServiceImpl implements UserService {
    ...
    public void sendSms(String phone) {
        ...
        redisTemplate.boundValueOps ("code_"+phone).expire (2, TimeUnit.MINUTES);//设置过期时间2分钟
        //3.发送给rabbitmq
    ...
    public void add(User user, String code) {
        //获取系统短信验证码
        String codeString= (String) redisTemplate.boundValueOps ("code_" + user.getPhone ()).get ();
        if(codeString==null){
        ...
        //用户是否存在不存在让用户名等于手机号(唯一)
        if (user.getUsername ()==null){
            user.setUsername (user.getPhone ());
        }
        User serachUser = new User ();
        serachUser.setUsername (user.getUsername ());
        if (userMapper.selectCount (serachUser)>0){//查询用户是否存在相同的记录
            throw new RuntimeException("该手机号已经注册");
        }
        user.setCreated (new Date ());
        user.setUpdated (new Date ());
        user.setPoints (0);//积分初始值
