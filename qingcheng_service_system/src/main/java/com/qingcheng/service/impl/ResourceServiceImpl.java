...
public class ResourceServiceImpl implements ResourceService {
    ...
    public List <Map> findAllResource() {
        ...
        return findResourceListByParentId (resourceList,0);//查询一级id
    ...
    public List <Map> findResourceListByParentId(List <Resource> resourceList, Integer ParentId) {
        ...
        for (Resource resource : resourceList) {//循环一级菜单
            if(resource.getParentId ().equals (ParentId)){
                Map map = new HashMap ();
                map.put ("id",resource.getId ());
                map.put ("resKey",resource.getResKey ());
                map.put ("resName",resource.getResName ());
                map.put ("children",findResourceListByParentId (resourceList, resource.getId ()));//递归
                list.add (map);
            }
